...
func (exporter *BaseHuaweiCloudExporter) listMetrics(namespace string) ([]metrics.Metric, map[string][]string) {
	...
	slog.Debug(fmt.Sprintf("[%s] Resource number of %s: %d", exporter.txnKey, namespace, len(allResourcesInfo)))

	if len(*metrics) > 0 {
		return *metrics, allResourcesInfo
	}
	slog.Debug(fmt.Sprintf("[%s] Start to getAllMetric from CES", exporter.txnKey))
	allMetrics, err := getAllMetric(exporter.ClientConfig, namespace)
	if err != nil {
		slog.Error(fmt.Sprintf("[%s] Get all metrics error: %s", exporter.txnKey, err.Error()))
		return nil, nil
	}
	slog.Debug(fmt.Sprintf("[%s] End to getAllMetric, Total number of of metrics: %d", exporter.txnKey, len(*allMetrics)))
...
func (exporter *BaseHuaweiCloudExporter) getLabelInfo(allResourcesInfo map[string][]string, metric metricdata.MetricData) *LabelInfo {
	...
	if len(labels) != len(values) {
		slog.Error(fmt.Sprintf("[%s] Inconsistent label and value: expected %d label %#v, but values got %d in %#v", exporter.txnKey,
...
func (exporter *BaseHuaweiCloudExporter) setProData(ctx context.Context, ch chan<- prometheus.Metric,
	...
	for _, metric := range dataList {
		...
		if err != nil {
			slog.Warn(fmt.Sprintf("[%s] Get data point error: %s, metric_name: %s, dimension: %+v", exporter.txnKey, err.Error(), metric.MetricName, metric.Dimensions))
			continue
		}
		...
		if err := sendMetricData(ctx, ch, proMetric); err != nil {
			slog.Error(fmt.Sprintf("[%s] Context has canceled, no need to send metric data, metric name: %s", exporter.txnKey, fqName))
		}
	}
}
...
func (exporter *BaseHuaweiCloudExporter) collectMetricByNamespace(ctx context.Context, ch chan<- prometheus.Metric, namespace string) {
	...
	if len(allMetrics) == 0 {
		slog.Warn(fmt.Sprintf("[%s] Metrics of %s are not found, skip.", exporter.txnKey, namespace))
		return
	}

	slog.Debug(fmt.Sprintf("[%s] Start to scrape metric data", exporter.txnKey))
	...
	for _, metric := range allMetrics {
		...
		if (len(tmpMetrics) == exporter.ScrapeBatchSize) || (count == len(allMetrics)) {
			...
			go func(tmpMetrics []metricdata.Metric) {
				...
				slog.Debug(fmt.Sprintf("[%s] Start to getBatchMetricData, metric count: %d", exporter.txnKey, len(tmpMetrics)))
	...
	wg.Wait()
	slog.Debug(fmt.Sprintf("[%s] End to scrape all metric data", exporter.txnKey))
}
...
func (exporter *BaseHuaweiCloudExporter) Collect(ch chan<- prometheus.Metric) {
	...
	slog.Debug(fmt.Sprintf("[%s] Start to collect data", exporter.txnKey))
	var wg sync.WaitGroup
	...
	wg.Wait()
	slog.Debug(fmt.Sprintf("[%s] End to collect data", exporter.txnKey))
}
...
func (exporter *BaseHuaweiCloudExporter) debugMetricInfo(md metricdata.MetricData) {
	...
	if err != nil {
		slog.Error(fmt.Sprintf("[%s] Marshal metricData error: %s", exporter.txnKey, err.Error()))
		return
	}
	slog.Debug(fmt.Sprintf("[%s] Get data points of metric are: %s", exporter.txnKey, string(dataJson)))
}
