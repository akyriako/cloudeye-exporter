...
func getCESClient(c *Config) (*golangsdk.ServiceClient, error) {
	...
	if clientErr != nil {
		slog.Error(fmt.Sprintf("Failed to get the NewCESV1 client: %s", clientErr.Error()))
...
func getELBlient(c *Config) (*golangsdk.ServiceClient, error) {
	...
	if clientErr != nil {
		slog.Error(fmt.Sprintf("Failed to get the NewLoadBalancerV2 client: %s", clientErr.Error()))
		return nil, clientErr
	}
...
func getAllRds(c *Config) (*rds.ListRdsResponse, error) {
	...
	if err != nil {
		slog.Error(fmt.Sprintf("Unable to get NewRDSV3 client: %s", err.Error()))
		return nil, err
	}
...
func getAllDcs(c *Config) (*dcs.ListDcsResponse, error) {
	...
	if err != nil {
		slog.Error(fmt.Sprintf("Failed to NewDCSServiceV1, error: %s", err.Error()))
		return nil, err
	}
...
func getAllDms(c *Config) (*dms.ListDmsResponse, error) {
	...
	if err != nil {
		slog.Error(fmt.Sprintf("Failed to NewDMSServiceV1, error: %s", err.Error()))
		return nil, err
	}
